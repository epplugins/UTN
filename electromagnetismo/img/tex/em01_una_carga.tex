...
\begin{document}
	...
	%
	% posicion q:
	\pgfmathsetmacro{\qx}{2.5}
	\pgfmathsetmacro{\qy}{2}
	\pgfmathsetmacro{\qz}{2.9}
	\pgfmathsetmacro{\px}{1}
	\pgfmathsetmacro{\py}{3}
	\pgfmathsetmacro{\pz}{3.25}
	...
	\begin{tikzpicture}[tdplot_main_coords]
		...
		\draw[axis] (0,0,0) -- (0,0,4.5)  node [pos=1.05] {$k$};

		%Charge
		\draw[dashed] (\qx,0,0) -- (\qx,\qy,0) node [pos=0, above left] {$x_0$};
		\draw[dashed] (0,\qy,0) -- (\qx,\qy,0) node [pos=-0.35, left] {$y_0$};
		...
		\draw[dashed] (\px,0,0) -- (\px,\py,0) node [pos=-0.1] {$x$};
		...
		\fill[white] (\qx,\qy,2.25) circle (2.3pt);
